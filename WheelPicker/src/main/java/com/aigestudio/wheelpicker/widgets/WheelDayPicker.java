...
public class WheelDayPicker extends WheelPicker implements IWheelDayPicker {
    ...
    private void updateDays() {
        ...
        if (mMonth == 11){
            mCalendar.set(mYear+1,0,0);
        }else{
            mCalendar.set(mYear,mMonth+1,0);
        }
        int days = mCalendar.get(Calendar.DAY_OF_MONTH);
        //Log.e("wstag",mYear+" "+mMonth+" "+days+"");
        //int days = mCalendar.getActualMaximum(Calendar.DATE);
...
//        }
        if (maxCalendar != null
                && maxCalendar.get(Calendar.YEAR) == mYear
                && maxCalendar.get(Calendar.MONTH) == mMonth){
            days = maxCalendar.get(Calendar.DAY_OF_MONTH);
        }
